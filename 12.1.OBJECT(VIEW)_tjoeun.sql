...
-- 뷰를 통해서 DML을 실행하면 실제 데이터가 담겨있는 테이블에 반영됨
...
-- INSERT
INSERT INTO VW_GROUP VALUES ('D3', 8000000, 40000000);
-- 오류 : 가상 열은 사용할 수 없습니다
...
--> 가지고 올때 서브쿼리의 값을 계산을 하고 가져온 것이기 때문 (테이블에 합계라는 것이 없음)

-- DELETE
DELETE FROM VW_GROUP
WHERE 합계 = 4970000;
-- 오류 : 뷰에 대한 데이터 조작이 부적합합니다
--> 여러명이 한 그룹에 속해 있기 때문에 GROUP BY절이 속한것은 모두 다 오류
...
-- INSERT
INSERT INTO VW_JOIN VALUES ('400', '이순신', '981112-1589476', 'J1', '총무부');
...
/*
    * VIEW 옵션
    ...
    - OR REPLACE : 기존에 동일한 뷰가 존재하면 덮어쓰기, 존재하지 않으면 새로 생성
    - FORCE | NOFORCE
        > FORCE : 서브쿼리에 기술된 테이블이 존재하지 않아도 뷰를 생성할 수 있다
        > NOFORCE : 서브쿼리에 기술된 테이블이 반드시 존재 해야만 뷰를 생성할 수 있다 (기본값)
    - WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로만 DML이 가능하도록 함
