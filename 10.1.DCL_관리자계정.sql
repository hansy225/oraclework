/*
    * <DCL : DATA CONTROL LANGUAGE> 데이터 제어 언어
        계정에게 시스템 권한 또는 객체 접근할 수 있는 권한 부여(GRANT)하거나 회수(REVOKE)하는 구문
        
        >> 시스템 권한 : DB에 접근하는 권한, 객체들을 생성할 수 있는 권한
            * 종류
                -  CRATE SESSION : 접속할 수 있는 권한
...
-- 1. SAMPLE 사용자 생성
CREATE USER SAMPLE IDENTIFIED BY 1234;  --> 접속 불가. 접근 권한이 없음

-- 2. 접속을 위한 CREATE SESSION 권한 부여
GRANT CREATE SESSION TO SAMPLE;     --> 접속만 가능
    -- 접속하여 테이블을 만들면 오류. 테이블을 만들 수 있는 권한은 없음
...
/*
    * 객체 접근 권한 종류
        ...
        [표현식]
        GRANT 권한의종류 ON 특정객체 TO 계정명;
*/
-- 5. SAMPLE 계정에게 TJOEUN 계정에 EMPLOYEE 테이블을 SELECT 할 수 있는 권한 부여
GRANT SELECT ON TJOEUN.EMPLOYEE TO SAMPLE;

-- 6. SAMPLE 계정에게 TJOEUN계정에 EMPLOYEE 테이블을 INSERT 할 수 있는 권한 부여
GRANT INSERT ON TJOEUN.EMPLOYEE TO SAMPLE;

-- * 권한 회수
--      REVOKE 회수할권한 ON 특정객체 FROM 계정명;
