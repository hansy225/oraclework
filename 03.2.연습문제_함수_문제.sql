...
SELECT EMP_NAME,
        EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) 생년,
        EXTRACT(MONTH FROM TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) 생월,
        EXTRACT(DAY FROM TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) 생일
FROM EMPLOYEE;
---------------------------------------
SELECT EMP_NAME 사원명,
...
FROM EMPLOYEE
WHERE ABS(CEIL(EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE))) >=20;
---------------------------------------
SELECT *
FROM EMPLOYEE
...
SELECT EMP_NAME, DEPT_CODE, 
        TO_CHAR(TO_DATE(LPAD(EMP_NO, 6), 'YYMMDD'), 'YY"년 " MM"월 " DD"일"') 생년월일,
        TO_CHAR(TO_DATE(LPAD(EMP_NO, 2),'RRRR')) - TO_CHAR(TO_DATE(LPAD(SYSDATE, 2), 'RRRR')) 나이
FROM EMPLOYEE;
---------------------------------------
SELECT EMP_NAME, DEPT_CODE,
...
SELECT EMP_ID, EMP_NAME, 
        SUBSTR(EMP_NO,1,6) "주민번호 앞자리", SUBSTR(EMP_NO,8) "주민번호 뒷자리",     --> 14 생략가능
        TO_NUMBER(SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8,14)) "주민번호 앞자리와 뒷자리의 합"
FROM EMPLOYEE
WHERE EMP_ID = '201';

-- 10. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT SUM((SALARY*NVL(1+BONUS, 1))*12) "보너스 포함 연봉"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
SELECT COUNT(*) 전체직원수,
        ...
        COUNT(CASE WHEN EXTRACT(YEAR FROM HIRE_DATE) = '2002' THEN 1 END) "2002년",
        COUNT(CASE WHEN EXTRACT(YEAR FROM HIRE_DATE) = '2003' THEN 1 END) "2003년",
        COUNT(CASE WHEN EXTRACT(YEAR FROM HIRE_DATE) = '2004' THEN 1 END) "2004년"
FROM EMPLOYEE;
---------------------------------------
SELECT COUNT(*) 전체직원수,
        COUNT(CASE WHEN TO_CHAR(HIRE_DATE, 'RRRR') = '2001' THEN 1 END) "2001년",
        COUNT(CASE WHEN TO_CHAR(HIRE_DATE, 'RRRR') = '2002' THEN 1 END) "2002년",
        COUNT(CASE WHEN TO_CHAR(HIRE_DATE, 'RRRR') = '2003' THEN 1 END) "2003년",
        COUNT(CASE WHEN TO_CHAR(HIRE_DATE, 'RRRR') = '2004' THEN 1 END) "2004년"
FROM EMPLOYEE;
