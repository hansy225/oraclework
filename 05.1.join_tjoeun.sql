/*
    ...
    - 관계형데이터베이스는 최소한의 데이터로 각각 테이블에 담고 있음 (중복을 최소화하기 위해)
...
/*
    * 포괄 조인 / 외부조인 (OUTER JOIN)
     두 테이블 간의 JOIN시 일치하지 않는 행동 포함시켜 조회
     단, 반드시 LEFT / RIGHT를 지정해야 됨 (기준이 되는 테이블을 지정해야 함)
*/
...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 연봉
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);   --> 기준이 되는 테이블의 반대편 테이블 컬럼의 뒤에 (+)를 붙인다
