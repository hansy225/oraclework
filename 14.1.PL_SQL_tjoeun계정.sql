/*
    ...
    * PL / SQL 구조
     ...
     - 실행부 (EXECUTANLE SECTION) : BEGIN으로 시작, SQL문 또는 제어문(조건문, 반복문)등의 로직을 기술하는 부분
...
-- 출력문 : HELLO ORACLE
BEGIN
    -- System.out.println("HELLO ORACLE");  --> 자바 같은의미
...
DECLARE
    EID NUMBER;
    ENAME VARCHAR2(20);
    PI CONSTANT NUMBER := 3.14; -- 'CONSTRAINT'를 'CONSTANT'로 수정
BEGIN
    EID := 800;
    ENAME := '배정남';
...
/*
        1.2 레퍼런스 스타일 변수 선언 및 초기화 (테이블의 컬럼의 데이터 타입을 참조하여 그 타입을로 지정)
            : 변수명 테이블명.컬럼명%TYPE;
*/
DECLARE
...
/*
        1.3 ROW 타입 변수
            : 테이블의 한 행에 대한 모든 컬럼 값을 한꺼번에 담을 수 있는 변수
            변수명 테이블명%ROWTYPE;
*/
-- 오류 : PL/SQL: ORA-00913: 값의 수가 너무 많습니다
DECLARE
    E EMPLOYEE%ROWTYPE;
BEGIN
    SELECT EMP_ID, EMP_NAME     --> 무조건 * 사용
    INTO E
    FROM EMPLOYEE
    WHERE EMP_ID = &사번;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('해당 사원의 급여 등급은 ' || GRADE || ' 등급입니다.');
END;
/
    ...
    4)
        ...
        SWITCH (변수) {           --> CASE
            ...
            DEFAULT :               --> ELSE
                실행내용2
*/
-- 사용자로부터 입력받은 사번의 사원을 부서코드를 보고 *부서이름*으로 출력하기
DECLARE
...
END;
/
-- ** 사용자로부터 입력받은 사번의 사원을 부서코드를 보고 *근무지역* 출력하기
    -- D5 = 일본
    -- D6 = 중국
    -- D7 = 미국
    -- D8 = 러시아
...
BEGIN
    ...
    NNAME := CASE EMP.NATIONAL_CODE
                        ...
                        ELSE '한국'
                    END;
    DBMS_OUTPUT.PUT_LINE(EMP.EMP_NAME || '는 ' || NNAME || ' 입니다.');
END;
/
-------------------------------------------------------------------------------------------------------
/*
...
*/
-- 1~5까지 1씩 증가하면서 출력
-- 1) IF 조건식 이용으로 빠져 나오기
DECLARE
    X NUMBER := 1;
BEGIN
    LOOP 
        DBMS_OUTPUT.PUT_LINE(X);     --> ++ 사용 불가
        X := X+1;
...
EXCEPTION
    ...
    --> 시스템 예외가 생각이 안난다면 OTHERS 작성해도 됨
END;
/
...
-- 사수번호가 200번은 여러명, 201번은 한명, 204번도 여러명
-- (EXCEPTION이 없으면) 오류 : 실제 인출은 요구된 것보다 많은 수의 행을 추출합니다
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
BEGIN
...
EXCEPTION
    WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('여러 행을 조회할 수 없습니다.');
    WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('조회 결과를 찾을 수 없습니다.');
END;
/
---------------------------------- 실습 문제 ----------------------------------
/*
